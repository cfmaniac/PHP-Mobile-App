...
class UserModel {
	...
	function create($info,$login = true){
		...
		$db->query("SELECT user_id, password FROM new_merchants WHERE email = '".mysql_real_escape_string($email)."'");
		if(mysql_num_rows($db->result) == 1){
			$this->msg = "Error! E-mail address is already in use.";
		}else{
			$query = $db->query("INSERT INTO new_merchants (name,email,password,status,created_ip) VALUES ('$name','$email','$password','$status','$created_ip')");
			if($query){
	...
	function update($info){
		...
		$query = "UPDATE new_merchants SET ".$sql." WHERE user_id = '".$this->user_id."'";
	...
	function login($email, $password){
		global $db;
		$sql = $db->query("SELECT user_id, password, name FROM new_merchants WHERE email = '".mysql_real_escape_string($email)."'");
		$this->ok = false;
		if(!$email || !$password){
	...
	function get_info($field = "*", $email = null){
		...
		$sql = $db->query("SELECT $field FROM new_merchants WHERE email = '$email'");
		$info = $db->fetch($sql);
	...
	function user_table($all = false, $start = 1, $limit = 20, $sort_by = "user_id DESC"){
		global $user, $db;
		$query = "SELECT * FROM new_merchants";
	...
	function user_info($user_id){
		global $user, $db;
		$query = "SELECT * FROM new_merchants WHERE user_id = '".$user_id."'"; 
		$result = $db->query($query);
	...
	function user_update($info, $user_id){
		global $db;
		$query = "UPDATE new_merchants SET status='".$info['status']."', name='".$info['name']."', email='".$info['email']."', admin='".$info['admin']."'";
		if($info['password']){
			$password = md5(mysql_real_escape_string($info['password']) . PASSWORD_SALT);
